...
| Background questions rather than a blocked UI | FIFO queueing, selected waiting-prompt cancellation, atomic queued-prompt editing, and durable restart recovery are visible and deterministically validated. Queue editing uses an explicit `EDIT QUEUED <id>` composer; SDK-native queue introspection remains optional adapter work. |
